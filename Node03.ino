...
#define NUM_LEDS    74 //amount of LED
...
void loop() {
  ...
  switch (gameState) {
    ...
    case 2:
      ...
      break;
    case 3:
      //code for displaying lights in mode 3
      if (state == 1) {
        countdownColor();
      }
      else {
...
void measureDistance() {
  ...
  digitalWrite(trigPin, LOW);
  // Reads the echoPin, returns the sound wave travel time in microseconds
  distance = rdPulseIn(echoPin, HIGH, 2) / 5.82; // distance in mm


  //calculating the running average over the last 4 values for #1
  if (distance < 1000) { //don't add values that are way out of bounce
  ...
  if (distance2 < 1000) { //don't add values that are way out of bounce
  ...
  if ((average > 0 && average < 1000) || (average2 > 0 && average2 < 1000)) {
    if (millis() > timer + 1999) { //more acurate than delay (500);
      timer = millis();
